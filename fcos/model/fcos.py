...
from .embeddings import COCO_EMBEDDINGS
...
class EfficientDetFCOS(HydraMixin, pl.LightningModule):
    ...
    def step(self, prefix: str, batch: Tuple[Tensor, Tensor], batch_nb: int):
        ...
        self.last_target = (img.clone(), {"coords": target[..., :4], "class": target[..., 4:], "names": COCO_EMBEDDINGS})
        ...
        with torch.no_grad():
            ...
            if not self.training:
                ...
                self.last_pred = (img, {"coords": bbox_coord, "score": bbox_score, "class": bbox_cls, "names": COCO_EMBEDDINGS})
